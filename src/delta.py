...
output_image = lambda n, v: cv2.imwrite(f'outputs/{n}.png', v)
...
def load_image(filename, desired_res):
    ...
    # Load the image
    img_ori = cv2.imread(f'img/{filename}')

    # Error checking
    if img_ori is None:
        error(f'Could not open or find the image: "{filename}".', 1)
...
def generate_output(img, count, method, filename, period):
    ...
    output_image(f'{name}_{method}_C{count}', border)
...
def main(args):
    ...
    if args[2].lower() == 'w':
        ...
        error(f'Undefined detection method: "{args[2]}"', 1)

    # Generate the output
    output = generate_output(output, colonies, args[2], args[1], (time.time() - start_time))

    # Visualize the final image
    cv2.imshow(f'Final Result: {colonies} Colonies found', output)
    cv2.waitKey()
...
if __name__ == '__main__':
    ...
    main(['delta.py', 'bacteria_light_6.jpeg', 'w'])
